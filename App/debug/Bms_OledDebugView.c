...
/**
 * @file App_OLED.c
 * @brief APP �?OLED 简单状态页�?
 *
 * 当前 OLED 只承�?bring-up 必需信息：BQ I2C 是否正常，以及成功读到的
...
/**
 * @brief �?4-bit 值转换为十六进制字符�?
 *
 * 避免为了显示 4 位十六进制值引�?sprintf�?
 */
static char Bms_OledDebugView_HexNibble(uint8_t value)
{
    value &= 0x0Fu;
    if (value < 10u)
    {
 ...
 */
static void Bms_OledDebugView_MakePowerConfigLine(uint16_t power_config, char *line)
{
    line[0] = 'P';
    line[1] = 'W';
    line[2] = 'R';
    line[3] = ':';
    line[4] = Bms_OledDebugView_HexNibble((uint8_t)(power_config >> 12u));
    line[5] = Bms_OledDebugView_HexNibble((uint8_t)(power_config >> 8u));
    line[6] = Bms_OledDebugView_HexNibble((uint8_t)(power_config >> 4u));
    line[7] = Bms_OledDebugView_HexNibble((uint8_t)power_config);
    line[8] = '\0';
}
 ...
 */
static void Bms_OledDebugView_Render(bool ok, bool power_valid, uint16_t power_config)
{
    char power_line[9];
    ...
    if (ok && power_valid)
    {
        Bms_OledDebugView_MakePowerConfigLine(power_config, power_line);
...
/**
 * @brief 初始�?OLED APP 页面�?
 */
void Bms_OledDebugView_Init(void)
{
    /* BQ 通信确认前默认显�?FAIL，方便上板排查�?*/
    Inf_OLED_Init();
    s_oled_ready = true;
    s_iic_status_valid = false;

    Bms_OledDebugView_ShowIicStatus(false);
}

/**
 * @brief 显示 BQ I2C 是否正常�?
 *
 * 连续相同状态不重复刷新，减�?OLED I2C 访问�?
 */
void Bms_OledDebugView_ShowIicStatus(bool ok)
{
    ...
    Bms_OledDebugView_Render(ok, ok && s_power_config_valid, s_last_power_config);
}

/**
 * @brief 显示 BQ I2C 状态和 Power Config�?
 *
 * Power Config 只有�?`ok=true` 时才有意义；失败时屏幕显示占位符�?
 * 内部保留上一次成功值以便后续恢复显示�?
 */
void Bms_OledDebugView_ShowBqIicPowerConfig(bool ok, uint16_t power_config)
{
    ...
    Bms_OledDebugView_Render(ok, ok, power_config);
}
