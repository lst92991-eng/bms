...
void App_BatMan_PrintFastMonitor(void)
{
    ...
    printf("BQFAST I:%ldmA V:%lu C:%u/%u/%u d:%u FET:%02x DSG:%u PDSG:%u SCD:%u OCD:%u CUV:%u XDSG:%u PF:%u\r\n",
           (long)current_ma,
           (unsigned long)stack_mv,
           (unsigned int)cell_min_mv,
           (unsigned int)cell_avg_mv,
...
void App_BatMan_PrintMonitorStopReason(void)
{
    ...
    printf("BQFAST关键量 I:%ldmA Stack:%lu Pack:%lu Cell:%u/%u/%u d:%u Temp:%d FET:%02x Req:%02x\r\n",
           (long)current_ma,
           (unsigned long)stack_mv,
...
void App_BatMan_PrintSnapshot(void)
{
    ...
    printf("BQ快照 通信故障:%u 电芯有效:%u 电流有效:%u 温度有效:%u 总故障:%u\r\n",
           ...
           fault_active ? 1u : 0u);
    printf("BQ逐串 C1:%u C2:%u C3:%u C4:%u C5:%u C6:%u 最低:%u 平均:%u 最高:%u 压差:%u 总压:%lu 包压:%lu 电流:%ld\r\n",
