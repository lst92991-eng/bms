...
#define APP_BATMAN_DM_CUV_THRESHOLD_2V83                (56u)   /* 56*50.6mV�?.83V，低�?APP 3.0V 实测硬底线�?*/
#define APP_BATMAN_DM_CUV_DELAY_1S                      (300u)  /* �?6.6ms + 300*3.3ms = 996.6ms�?*/
#define APP_BATMAN_DM_CUV_RECOVERY_HYS_200MV            (4u)    /* 4*50.6mV�?02mV，避免低压打嗝�?*/
#define APP_BATMAN_DM_OCC_THRESHOLD_6A                  (15u)   /* 15*2mV/5mΩ=6A，作�?SC8815 充电限流后备�?*/
#define APP_BATMAN_DM_OCC_DELAY_426MS                   (127u)  /* U1 最大档附近，约 6.6ms + 127*3.3ms = 425.7ms�?*/
#define APP_BATMAN_DM_OCD1_THRESHOLD_14A                (35u)   /* 35*2mV/5mΩ=14A，位�?APP 12A �?SCD 16A 之间�?*/
#define APP_BATMAN_DM_OCD1_DELAY_300MS                  (89u)   /* �?6.6ms + 89*3.3ms = 300.3ms�?*/
#define APP_BATMAN_DM_OCD2_THRESHOLD_15A2               (38u)   /* 38*2mV/5mΩ=15.2A，SCD 前的快速过流层�?*/
#define APP_BATMAN_DM_OCD2_DELAY_80MS                   (22u)   /* �?79.2ms�?*/
#define APP_BATMAN_DM_SCD_THRESHOLD_80MV                (0x04u) /* 80mV/5mΩ�?6A，给12A放电测试留余量�?*/
#define APP_BATMAN_DM_SCD_DELAY_400US_TEST              (0x1Cu) /* 近似 400us，实际约 405us�?*/
#define APP_BATMAN_DM_OCC_RECOVERY_NEG_200MA            ((uint16_t)0xFF38u) /* I2 -200mA，回到轻微放�?零电流再恢复�?*/
...
#define APP_BATMAN_DM_OTD_THRESHOLD_60C                 (60u)   /* 与放电温度上限一致，BQ 直接兜底�?DSG�?*/
 ...
 *
 * APP 层只表达业务配置项，I2C、Data Memory 校验和及时序细节�?INT 层负责�?
 */
...
bool Bms_BqConfigPort_ApplyConfig(void)
{
    ...
    /*
     * 实板低边采样电阻�?5mΩ。把 BQ 自身�?CC Gain/Capacity Gain
     * 配准到硬件值后，APP 层电流读数保�?1:1，避�?SOC 和日志使用临时比例�?     */
    ...
    /*
     * 保护配置与告警掩码决�?BQ 上报哪些事件，以及哪些事件进�?     * BQ 自己的保�?FET 判定路径�?
     */
    ...
    /*
     * FET_INIT_OFF + HOST_FET_EN 让主充放�?MOS 上电后保持关断，
     * 直到 MCU 明确�?host FET control 释放。当�?bring-up 阶段只关不断开�?
     * 不自动释�?CHG/DSG�?
     */
    ...
    /*
     * 关闭 BQ 自主均衡，允�?MCU 后续�?CB_ACTIVE_CELLS 明确指定均衡串�?
     */
    ...
    /*
     * FET protection routing 决定哪些安全事件会关�?CHG/DSG�?
     * 这些值和 FET_OPTIONS 放在同一段，便于上板前整体审�?FET 托管�?
     */
...
/**
 * @brief �?BQ 进入 FET 固件控制态，并保持主充放�?MOS 关断�?
 *
 * 默认安全策略是“能通信、能采样、能均衡，但主功率路径不自动接通”�?
 * 这里先确�?FET_EN �?firmware control，再�?FET_CONTROL off mask�?
 * 明确关断 CHG/DSG/PCHG/PDSG。后续若要放开主功�?MOS，应新增单独业务入口�?
 */
...
bool Bms_BqConfigPort_SetPreDischargeFet(bool charge_enable)
{
    ...
    /*
     * FET_CONTROL �?1 是强制关断。BQ76952 �?PDSG_EN 会在 DSG 被允许打开�?
     * 自动先走 PDSG 预放电，再由器件切到 DSG；这里不能把 DSG_OFF �?1�?
     * 否则器件永远收不到“允许放电”的请求�?
     */
    if (App_BatMan_WriteMainFetControl(off_mask) == INT_BQ76952_OK)
    {
...
bool Bms_BqConfigPort_TestPreDischargeOnly(void)
{
    ...
    /*
     * 串口定位专用：只�?PDSG_OFF，保�?DSG_OFF，确�?BQ 和硬件是�?     * 真的能单独拉起预放电通道。FET_CONTROL 只表达关断掩码，随后还需
     * ALL_FETS_ON 释放 FET_INIT_OFF/host off latch，否�?PDSG 可能保持关闭�?     */
    if ((App_BatMan_WriteMainFetControl(off_mask) == INT_BQ76952_OK) &&
...
{
    return Bms_BqConfigPort_ApplyConfig();
}

Int_BQ76952_StatusTypeDef App_BatMan_KeepMainFetsOff(void)
{
    return Bms_BqConfigPort_KeepMainFetsOff();
}

bool App_BatMan_SetMainFets(bool charge_enable, bool discharge_enable)
{
    return Bms_BqConfigPort_SetMainFets(charge_enable, discharge_enable);
}

bool App_BatMan_SetPreDischargeFet(bool charge_enable)
{
    return Bms_BqConfigPort_SetPreDischargeFet(charge_enable);
}

bool App_BatMan_TestPreDischargeOnly(void)
{
    return Bms_BqConfigPort_TestPreDischargeOnly();
}
