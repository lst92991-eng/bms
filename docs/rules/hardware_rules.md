...
2026-06-25 已重新读取新旧项目的网表和原理图 PDF，本文档中的 BQ/SC 关键硬件事实以这些资料为依据：

- 新项目 BMS 板：`full_netlist (4).csv`、`SCH_机器人BMS板_2026-06-17.pdf`。
- 新项目控制板：`full_netlist (5).csv`、`SCH_Schematic1_2026-06-17.pdf`。
- 旧项目控制板：`旧项目信息/old/full_netlist (4).csv`、`旧项目信息/old/SCH_UPS控制板_2026-06-18.pdf`。
- 旧项目电池板：`旧项目信息/old/full_netlist (5).csv`、`旧项目信息/old/SCH_6节14500板_2026-06-18.pdf`。
...
- SC8815 VBATS 外部分压名义目标：`25.2V`；实际截止电压受 SC8815 基准和电阻精度影响，必须上板测量。
...
- 按数据手册公式 `VBAT = 1.2V * (1 + R17 / R18)`，上述阻值的名义目标为 `25.2V`。旧网表中的 `R17/R18=0Ω` 仅作为改板前历史证据，不再代表当前实物。
- SC8815 输入侧限流 `IBUS` 与电池侧充电限流 `IBAT` 必须分开处理，禁止把电池侧 `5A` 目标直接套到输入侧。
- 软件默认输入限流：`1500mA`。
- 软件默认电池充电限流：先按 `3000mA` 保守启动。
- Bring-up 输入限流：`500mA`。
- Bring-up 电池充电限流：`300mA` 或 `500mA`，确认链路稳定后再升到 `3000mA`。
- 软件硬上限：输入 `3000mA`，电池充电 `5000mA`。
- 电池侧充电电流目标：用户已于 2026-06-25 确认 `5A`；为保险，初期运行先按 `3A`。
- 基于当前 `R5/R14=10mΩ`、`IBUS_RATIO=3`、`IBAT_RATIO=6`，寄存器量程约为 `IBUS=3A`、`IBAT=6A`；因此 `IBAT=5A` 在寄存器量程内，`IBUS` 仍保持 `3A` 上限。
...
- BQ76952 默认 I2C CRC 开启；INT 层必须保留 CRC 支持与开关。
- `PB6 = I2C1_SCL`，`PB7 = I2C1_SDA`。
- `PB4 = BMS_INT/ALERT`，外部上拉 `R56=10k`。
- `PB3 = BMS_WAKE`，走 TS2 唤醒路径。
- `BMS_CHIP_SHUT / RST_SHUT` 通过跨板接口和开关相关电路连接，短复位/长关断时序后续放 BSP/GPIO 层细化。
...
- 低边电流采样：`R18=5mΩ/6W`，位于 `GND` 与 `BMS_OUT-` 之间。用户已于 2026-07-08 确认按 `5mΩ` 处理；若旧网表或迁移说明仍写 `0.5mΩ`，以后续实物/用户确认值为准。
- `TS1/TS3` 为靠近低边采样电阻的热敏电阻，用于采样电阻温度补偿/热保护；NTC 型号、阻值、B 值和分压参数仍需确认。
- 设计放电电流目标：用户已于 2026-06-25 确认 `10A`，方向为 `VBAT -> BMS_OUT-`；BQ76952 过流/短路保护阈值需按 `10A` 工作点规划，但必须等电流方向、采样校准和保护延时确认后再写死到代码。
...
- BQ76952 子命令时序、带数据 subcommand 写入和 CRC 通信需要在实物 bring-up 时示波/读回确认。
- OLED I2C 地址。
- EEPROM 页大小、写周期、地址宽度。
